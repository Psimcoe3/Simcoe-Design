...
try {
  ...
  $tab2d = $window.FindFirst(
    ...
    (New-Object System.Windows.Automation.AndCondition(
      ...
      (New-Object System.Windows.Automation.PropertyCondition([System.Windows.Automation.AutomationElement]::NameProperty, '2D Plan View'))
    ))
  )
  if ($tab2d) {
    $sel = $tab2d.GetCurrentPattern([System.Windows.Automation.SelectionItemPattern]::Pattern)
    $sel.Select()
  ...
  for ($i=0; $i -lt $all.Count; $i++) {
    $el = $all.Item($i)
    if ($el.Current.AutomationId -in @('PlanCanvas','Viewport','ViewTabs','PlanScrollViewer','LibraryListBox','LayerListBox','PropertiesPanel','MainContentGrid','PdfControlsPanel')) {
