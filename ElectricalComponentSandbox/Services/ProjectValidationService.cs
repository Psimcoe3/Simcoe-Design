...
public class ProjectValidationService
{
    ...
    public ProjectValidationReport Validate(ProjectValidationInput input)
    {
        ...
        // 4. NEC 110.26 panel working clearance and dedicated equipment space
        foreach (var panel in input.Components.OfType<PanelComponent>())
        {
            var equipment = BuildPanelClearanceInput(panel);

            if (panel.HasWorkingClearanceData)
            {
                checksRun++;
                var clearance = ElectricalRoomClearanceService.CheckClearance(equipment);
                if (!clearance.FullyCompliant)
                {
                    findings.Add(new ValidationFinding
                    {
                        Id = $"V-{findingIndex++:D4}",
                        Category = FindingCategory.EquipmentClearance,
                        Severity = FindingSeverity.Error,
                        Title = "Working Clearance Deficient",
                        Description = $"{panel.Name}: {string.Join("; ", clearance.Violations)}",
            ...
            if (panel.HasDedicatedSpaceData)
            {
                checksRun++;
                var dedicatedSpace = ElectricalRoomClearanceService.CheckDedicatedSpace(
                    equipment,
                    panel.ProvidedDedicatedSpaceWidthInches,
                    panel.ProvidedDedicatedSpaceDepthInches,
                    panel.ProvidedDedicatedSpaceHeightFeet);

                if (!dedicatedSpace.Compliant)
                {
                    findings.Add(new ValidationFinding
                    {
                        Id = $"V-{findingIndex++:D4}",
                        ...
                        Title = "Dedicated Equipment Space Deficient",
                        Description = $"{panel.Name}: {string.Join("; ", dedicatedSpace.Violations)}",
                        ComponentId = panel.Id,
                        NecReference = "NEC 110.26(E)",
                    });
                }
            }
        }

        // 5. Connector-based electrical circuit topology
    ...
    private static ElectricalRoomClearanceService.EquipmentClearance BuildPanelClearanceInput(PanelComponent panel)
    {
        return new ElectricalRoomClearanceService.EquipmentClearance
        {
            Id = panel.Id,
            Name = panel.Name,
            NominalVoltage = panel.NominalVoltage,
            Condition = panel.WorkingClearanceCondition,
            EquipmentWidthInches = panel.Parameters.Width,
            EquipmentHeightInches = panel.Parameters.Height,
            EquipmentDepthInches = panel.Parameters.Depth,
            ProvidedClearanceDepthFeet = panel.ProvidedWorkingClearanceDepthFeet,
            ProvidedClearanceWidthInches = panel.ProvidedWorkingClearanceWidthInches,
            ProvidedClearanceHeightFeet = panel.ProvidedWorkingClearanceHeightFeet,
        };
    }
}
